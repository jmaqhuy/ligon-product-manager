...
const PERMISSIONS: Record<Action, Role[]> = {
  ...
  update_production: ["employee", "worker", "manager", "boss"],
  manage_orders: ["employee", "worker", "manager", "boss"],
  manage_shipments: ["employee", "worker", "manager", "boss"],
  change_fulfillment_type: ["manager", "boss"],
  edit_own_profile: ["employee", "manager", "boss"],
  manage_production_layouts: ["manager", "boss"],
  verify_production_layout: ["worker", "manager", "boss"],
  create_production_layout: ["employee", "manager", "boss"],
};
...
export function canManageUser(actorRole: Role, targetRole: Role): boolean {
  ...
  if (actorRole === "manager" && (targetRole === "employee" || targetRole === "worker")) return true;
  return false;
}
