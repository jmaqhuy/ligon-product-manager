...
export function PhotoGallery({
  ...
  const displayCells = Array.from({ length: 5 }).map((_, i) => images[i] || null);
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <h3 className="text-xs font-bold text-foreground uppercase tracking-wider">Gallery Images</h3>
          {statusBadge(pStatus, photoStatusLabels)}
        </div>
        <div className="flex items-center gap-1.5">
          {canManagePhotos && pStatus === "not_requested" && (
            <Button size="sm" className="h-6 text-[10px] px-2" onClick={() => handleUpdateListing({ photoStatus: "awaiting_photos" })} disabled={saving}>
              <ImageIcon className="h-3 w-3 mr-1" /> Yêu cầu làm ảnh
            </Button>
          )}

          {pStatus === "awaiting_photos" && !assigneeId && (
            <Button size="sm" className="h-6 text-[10px] px-2" onClick={() => handleUpdateListing({ photoAssigneeId: session?.user?.id })} disabled={saving}>
              <Check className="h-3 w-3 mr-1" /> Nhận
            </Button>
          )}

          {pStatus === "awaiting_photos" && assigneeId === session?.user?.id && (
            <>
              <Button size="sm" variant="outline" className="h-6 text-[10px] px-2" onClick={() => handleUpdateListing({ photoAssigneeId: null })} disabled={saving}>
              ...
              <Button size="sm" className="h-6 text-[10px] px-2" onClick={() => {
                ...
                <ShoppingBag className="h-3 w-3 mr-1" /> Nộp
              </Button>
            </>
          )}

          {pStatus === "revision_requested" && assigneeId === session?.user?.id && (
            <Button size="sm" className="h-6 text-[10px] px-2" onClick={() => handleUpdateListing({ photoStatus: "pending_approval" })} disabled={saving}>
              <ImageIcon className="h-3 w-3 mr-1" /> Nộp lại
            </Button>
          )}

          {canManagePhotos && pStatus === "pending_approval" && (
            <>
              <Button size="sm" variant="outline" className="h-6 text-[10px] px-2 border-red-200 text-red-700 hover:bg-red-50" onClick={() => {
              ...
              <Button size="sm" className="h-6 text-[10px] px-2 bg-green-600 hover:bg-green-700 text-white" onClick={() => handleUpdateListing({ photoStatus: "approved" })} disabled={saving}>
      ...
      {pStatus === "revision_requested" && listing?.photoRevisionNote && (
      ...
      <div className="grid grid-cols-5 gap-2 pb-2">
        {displayCells.map((img, i) => {
          if (i === 4 && totalImages > 5) {
            return (
              <ImagePreviewDialog key={i} images={images as string[]} initialIndex={i}>
                <div className="aspect-square bg-muted/40 border border-dashed border-muted-foreground/30 rounded flex items-center justify-center overflow-hidden cursor-pointer hover:ring-2 hover:ring-primary/50 transition-all relative">
                  <img src={driveToThumbnailUrl(img as string, 150)} alt={`Ảnh ${i + 1}`} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                    <span className="text-white font-semibold text-sm">+{totalImages - 4}</span>
                  </div>
                </div>
              </ImagePreviewDialog>
            );
